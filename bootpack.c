...
{
	struct FIFO32 fifo;
	struct TIMER *timer_ts, *timer_1s;
	...
	for (;;) {
		++count;
		putfonts_sht(sht_back, 0, 144, COL8_FFFFFF, COL8_008484, " ", 1);
		...
		if (fifo32_status(&fifo) == 0) {
			...
			if (i == 2) {
				...
				mysprintf(s, "%10d", count - count0);
				putfonts_sht(sht_back, 0, 128, COL8_FFFFFF, COL8_008484, s, 10);
				count0 = count;
				timer_settime(timer_1s, 100);
			}
		}
